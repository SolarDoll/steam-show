...
(function(){
  ...
  /* спеки под букинг: длительность / формат / состав (на заглушках — поправим по факту) */
  var SPECS={
    dragon: {Duration:'10–15 min · adaptable', Format:'Outdoor',          Cast:'Scalable'},
    fire:   {Duration:'10–15 min · adaptable', Format:'Outdoor',          Cast:'Scalable'},
    ledfire:{Duration:'10–15 min · adaptable', Format:'Outdoor',          Cast:'Scalable'},
    led:    {Duration:'10–15 min · adaptable', Format:'Indoor & outdoor', Cast:'Scalable'},
    stilts: {Duration:'Roaming or set act',    Format:'Indoor & outdoor', Cast:'Scalable'}
  };
  ...
  function specsLI(id){var sp=SPECS[id]||{};return Object.keys(sp).map(function(k){
    return '<li><span class="sk">'+k+'</span><span class="sv">'+sp[k]+'</span></li>';}).join('');}
  ...
  function overlay(id,i,mode){
    ...
    return '<article class="ocard" data-id="'+id+'" style="--c:var('+CVAR[id]+')">'
      ...
      +'<div class="info"><h3>'+s.name+'</h3>'
        +'<p class="odesc">'+s.desc+'</p>'
        +'<ul class="ospecs">'+specsLI(id)+'</ul>'
        +'<ul class="tags">'+tagsLI(id)+'</ul>'+vm(id)+'</div></article>';
  }
